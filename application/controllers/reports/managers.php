...
class Managers extends Admin_Controller
{
    ...
    function mail(){
        ...
        if($regionslist){
            ...
            foreach($regionslist as $region){
                ...
                $query = $this->reports_m->get_purchase_report_by_region($region_id);
                ...
                if($query){
                    ...
                    if($emailTo){
                        ...
                        if($file_created){
                            ...
                            $this->email->attach('./exports/managers/'.$region_name.'/Stockist_purchase_report.xls');
                            $this->email->subject($region_name.' Stockist Purchases Report');
                            $this->email->message('Kindly find attached weekly report for purchases made by stockists in your region.');
    ...
    function createExcel($query,$region_name){
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:N3')->applyFromArray($styleArray);
        $objPHPExcel->getActiveSheet()->getStyle('A1:N3')->applyFromArray($styleArray1);
        $objPHPExcel->getActiveSheet()->getStyle('A1:N3')->applyFromArray($styleArray12);
        $objPHPExcel->getActiveSheet()->getStyle('A4:N4')->applyFromArray($styleArrayHeaders);
        ...
        foreach($query->result() as $data)
        {
            ...
            $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':N'.$row)->applyFromArray($styleArray1);
            $row++;
        }
