...
class Managers extends MY_Controller{
    ...
    function index(){
        ...
        $data['title'] = "Area Sales Managers";
    ...
    function datatables(){
        $this->datatables->select('area_managers.id AS id,area_managers.name as name,mobile,email,division,regions.name AS region,area_managers.modified As modified,area_managers.created As created')
            ->unset_column('id')
            ->add_column('actions', get_area_managers_buttons('$1'), 'id')
            ->from('area_managers')
            ->join('regions', 'area_managers.region_id = regions.id');
    ...
    function add(){
        ...
        $this->form_validation->set_rules('name', 'Manager Name', 'required|max_length[60]|is_unique[area_managers.name]');
        $this->form_validation->set_rules('mobile', 'Manager Mobile', 'required|numeric|max_length[12]|is_unique[area_managers.mobile]');
        $this->form_validation->set_rules('email', 'Email', 'required|valid_email|max_length[100]|is_unique[area_managers.email]');
    ...
    function import(){
        ...
        $data['user_role'] = $this->session->userdata('role');
        $data['title'] = "Import Area Managers";
        $this->load->view('templates/header', $data);
    ...
    function import_excel($fileName){
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            if ($managerDivision != null AND $managerName!=null AND $managerMobile!=null AND $managerRegion!=null AND $managerEmail !=null) {
                ...
                $existRegion = $this->regions_m->get_region_by_name($managerRegion);
                ...
                if($existRegion){
                        ...
                        if($existsMobile OR $valid_mobile === false){
                            ...
                            if(valid_email($managerEmail)===false){
                                ...
                                if($existsEmail===true){
                                    ...
                                    $manager_added = $this->managers_m->add_manager(ucwords($managerName),$managerMobile,strtolower($managerEmail),strtoupper($managerDivision),$existRegion->id);
                    ...
                    $notAdded =$notAdded. ' | '.$managerName;
                    //log the region as not added
                    log_message('info','MANAGER NOT ADDED! '.$managerDivision.' '.$managerName.' '.$managerMobile.' '.$managerEmail.' REGION doesnt Exist ');
                }
